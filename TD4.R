library(MASS)
reg <- read.table('D:/文档/SY19/TD/TD4/TPN1_a22_reg_app.txt', header = TRUE)
class <- read.table('D:/文档/SY19/TD/TD4/TPN1_a22_clas_app.txt', header = TRUE)
...
validationplot(pcr_model, val.type="MSEP")#D'après le graphe on constate qu'il faut toutes les variables pour obtenir le meilleur modèle


#modele lineare#
reg.lm <- lm(formula = y ~., data = x.app_reg)
summary(reg.lm)#On a déjà une p-value qui est assez petite en une R-Square assez grande
res_std <- rstandard(reg.lm)
plot(x = y.app_reg, y = res_std)#Les résidus stadards parraissent pas mal
abline(0, 0)
...
# End Code Chenxin -------------------------------------------------------------
#Subset selection
#Vu qu'il existe trop de variables, nous pourrions pas utiliser la méthode best subset selection
...
#Puisque la méthode backward selection est mieux, 
